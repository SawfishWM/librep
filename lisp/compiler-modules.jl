...
(define-structure compiler-modules (export comp-variable-p
  ...
  (defun comp-symbol-value (var)
    (if (and (symbolp var) (special-variable-p var))
	(symbol-value var)
  ...
  (defun comp-macroexpand-1 (form)
    (when (consp form)
      (let* ((def (assq (car form) comp-macro-env))
	     ...
	     (macro-environment comp-macroexpand-1))
	(if def
  ...
  (defun comp-compile-module-body (body opened accessed)
    (let
	((comp-open-modules opened)
	 (comp-accessed-modules accessed)
	 ...
	 next-body)
      ...
      (catch 'out
	(mapc (lambda (struct)
		(when (get struct 'compiler-module)
...
;;; module compilers
  ...
  (defun compile-structure-def (name sig body)
	...
	(let
      ...
      (setq body (comp-compile-module-body body opened accessed))
