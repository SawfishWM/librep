...
(put 'load-file 'repl-command
     (lambda (file . args)
       (eval `(,load ,file ,@args) *repl-in-struct*)))
(put 'load-file 'repl-help "\"FILENAME\"")

(put 'open 'repl-command
     (lambda structs
       (eval `(,%open-structures (,quote ,structs)) *repl-in-struct*)))
(put 'open 'repl-help "STRUCT ...")

(put 'access 'repl-command
     (lambda structs
       (eval `(,%access-structures (,quote ,structs)) *repl-in-struct*)))
...
(put 'dis 'repl-command
     (lambda (arg)
       ...
       (disassemble (eval arg *repl-in-struct*))))
...
(put 'compile-proc 'repl-command
     (lambda args
       (require 'compiler)
       (mapc (lambda (arg)
	       (compile-function (eval arg *repl-in-struct*)))
	     args)))
...
(put 'describe 'repl-command
     (lambda (name)
       (require 'lisp-doc)
       (let* ((value (eval name *repl-in-struct*))
...
(put 'apropos 'repl-command
     (lambda (re)
       ...
       (let ((funs (apropos re (lambda (x)
				 (condition-case nil
				     (progn
				       (eval x *repl-in-struct*)
				       t)
				   (void-value nil))))))
	 (mapc (lambda (x)
		 (let* ((val (eval x *repl-in-struct*)))
...
(put 'time 'repl-command
	 ...
	 (setq ret (eval form *repl-in-struct*))
